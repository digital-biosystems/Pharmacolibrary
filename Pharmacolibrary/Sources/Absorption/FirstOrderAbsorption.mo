...
model FirstOrderAbsorption "enteral route: absorption lag Tlag then first-order uptake ka from a depot (lumen)"
  // Tlag (default 10 min) is applied by the dose schedule, not here: PeriodicDose starts its
  // pulses Tlag later. Delaying the pulse with a FixedDelay instead hid the pulse edges from
  // the solver, which could step across a short (1 s) oral bolus and lose part of the dose.
  extends Pharmacolibrary.Sources.Absorption.PartialAbsorption(Tlag = 600);
...
equation
  connect(mdot, variableDose.massFlow) annotation(
    Line(points = {{-100, 40}, {-33, 40}, {-33, 25}}, color = {0, 0, 127}));
